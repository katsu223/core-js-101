...
function chainPromises(array, action) {
  ...
  async function resolvePromises(arr, index = 0) {
    if (arr.length === index) return true;

    try {
      const resolvedValue = await array[index];

      resolvedArr.push(resolvedValue);
    } catch (err) {
      rejectedArr.push(err);
    }

    await resolvePromises(arr, index + 1);

    return resolvedArr;
  }

  return new Promise((resolve) => {
    resolvePromises(array).then((resultArray) => resolve(resultArray.reduce(action)));
  });
}
